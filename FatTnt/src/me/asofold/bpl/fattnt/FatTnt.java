...
import me.asofold.bpl.fattnt.effects.DamageProcessor;
import me.asofold.bpl.fattnt.effects.ExplosionManager;
import me.asofold.bpl.fattnt.events.FatExplodeEvent;
import me.asofold.bpl.fattnt.propagation.Propagation;
import me.asofold.bpl.fattnt.propagation.PropagationFactory;
import me.asofold.bpl.fattnt.scheduler.ProcessHandler;
import me.asofold.bpl.fattnt.scheduler.ScheduledEntitySpawn;
import me.asofold.bpl.fattnt.scheduler.ScheduledExplosion;
...
//	ExplosionPrimeEvent waitingEP = null;
	...
	final void onEntityCombust(final EntityCombustEvent event){
		...
		final ExplosionSettings es = settings.getApplicableExplosionSettings(loc.getWorld().getName(), null);
		// TODO: use item ?
		if (!es.itemTnt.value || es.preventExplosions.value) return;
		event.setCancelled(true);
		// TODO might have to be scheduled
		item.remove();
		if (es.scheduleEntities.value){
			schedulers.spawnEntities.addEntry(ExplosionManager.getScheduledTnt(item));
			checkSchedulers();
		}
...
//	}
	...
	public void createExplosion(World world, double x, double y, double z, float radius, boolean fire, Entity explEntity, EntityType entityType){
		...
		float realRadius = radius*explSettings.radiusMultiplier.value;
		List<Entity> nearbyEntities;
		long ms = System.nanoTime();
		if (explEntity==null) nearbyEntities = Utils.getNearbyEntities(world, x,y,z, realRadius*explSettings.entityRadiusMultiplier.value);
		else nearbyEntities = explEntity.getNearbyEntities(realRadius, realRadius, realRadius*explSettings.entityRadiusMultiplier.value);
